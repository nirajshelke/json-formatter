<template>
    <div role="group" aria-labelledby="group-label" part="dual-listbox">
            ...
            <div class="slds-dueling-list" onfocusin={handleFocus} onfocusout={handleBlur}>
                ...
                <div class={computedLeftColumnClass}>
                    ...
                    <div class={computedListboxContainerClass} style={computedColumnStyle}>
                        <ul data-source-list id="source-list" aria-describedby="keyboard-interacton" aria-labelledby="source-list-label" aria-multiselectable="true"
                            class="slds-listbox slds-listbox_vertical" role="listbox" part="listbox" aria-disabled={ariaDisabled}>
                            <template for:each={computedSourceList} for:item="option" for:index="index">
                                <li key={option.value} role="presentation" class="slds-listbox__item">
                                    <div class={option.classList} aria-selected={option.selected} role="option" tabindex={option.tabIndex} data-index={index}
                                        data-value={option.value} data-type={computedSourceListId} onclick={handleOptionClick} onkeydown={handleOptionKeyDown}>
                    ...
                    <div if:true={showActivityIndicator}>
                        <lightning-spinner size="small" variant="brand" alternative-text={i18n.loadingText}></lightning-spinner>
                    </div>
                </div>
                ...
                <div class="slds-dueling-list__column slds-dueling-list__column_responsive">
                    <span class="slds-form-element__label" id="selected-list-label">{selectedLabel}</span>
                    <div class={computedListboxContainerClass} style={computedColumnStyle}>
                        <ul data-selected-list id="selected-list" aria-describedby="keyboard-interacton" aria-labelledby="selected-list-label" aria-multiselectable="true"
                            class="slds-listbox slds-listbox_vertical" role="listbox" part="listbox" aria-disabled={ariaDisabled}>
                ...
                <template if:false={disableReordering}>
                    <div class="slds-dueling-list__column">
                        <lightning-button-icon icon-name="utility:up" variant="container" onclick={handleUpButtonClick} disabled={disabled} title={upButtonLabel}
                            alternative-text={upButtonLabel}>
                        </lightning-button-icon>
                        <lightning-button-icon icon-name="utility:down" variant="container" onclick={handleDownButtonClick} disabled={disabled} title={downButtonLabel}
        ...
        <template if:true={errorMessage}>
            <span id="error-message" class="slds-has-error slds-form-element__help" aria-live="assertive">{errorMessage}</span>
        </template>
    </div>
</template>
